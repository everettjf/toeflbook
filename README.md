...
- 📘 **The Book** — a 130-day, evidence-based study plan (`book/`) → builds to a single PDF.
- 🎮 **The Game** — a browser app with **20 mini-game engines** that train Listening, Speaking, Reading & Writing by *playing* (`game/`).

> **Start:** 2026‑05‑18 · **Exam:** 2026‑09‑26 · **Target:** 100+ (CEFR C1) · **Daily commitment:** 90 min
...
One game a day for 130 days. Each day is mapped to a skill and a game **engine**; engines **rotate** so the four weeks stay fresh, and a 🎲 **Shuffle** button swaps to another engine that fits the same content. Your progress, best score, and **mid-game resume** are saved in the browser; the calendar shows ✅ + score per day.
...
All 130 days are built: **200 unique academic words**, listening/dictation/echo sets, reading passages, grammar drills, writing tasks, and **4 full mock tests** (reading / listening / speaking / writing sections). 3D engines fall back to a quick-match mode when WebGL is unavailable; speech features need Chrome/Edge desktop and degrade gracefully elsewhere.
...
A 130-day plan for the **redesigned 2026 exam** — shorter (90 min), adaptive Reading/Listening, new Speaking (Listen & Repeat + Interview) and Writing (Build a Sentence + Email + Academic Discussion), AI scoring that penalizes templated answers.
...
It runs on a fixed **90-minute daily template** (Anki + shadowing as the non-negotiable floor, plus a rotating Main/Secondary skill block and a nightly error log) across **9 bi-weekly chapters + a final taper**. Full methodology, daily schedule, checkpoints, and the free resource catalog live in [`book/README.md`](book/README.md) and [`book/resources.md`](book/resources.md).
